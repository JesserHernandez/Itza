...
function destroy(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(
                route("product_materials.destroy", { product_material: id })
            );
            Swal.fire({
                title: "¡Eliminado!",
...
<template>
    <AppLayout title="Materiales" :href="route('dashboard')">
        <HeaderAdmin
            icon="/icons/icons-ceramics/ceramic-mingle-dark-icon.svg"
            title="Materiales"
            paragraph="Este espacio está hecho exclusivamente para gestionar los materiales de los productos que ofreces en tu tienda.
            Aquí puedes agregar, editar o eliminar materiales según sea necesario."
        />

        <div class="content">
            <div class="header-content-index">
                <div class="select-container">
                    <label for="productMaterial">Registros por página:</label>
                    <select
                ...
                <NavLink
                    class="btn-class"
                    :href="route('product_materials.create')"
                >
                    <span class="name-link"> Nuevo material </span>
                    <img
            ...
            <div class="container-table">
                <table class="table">
                    ...
                    <tbody>
                        <tr
                            v-for="productMaterial in productMaterials.data"
                            :key="productMaterial.id"
                        >
                            <td>{{ productMaterial.name }}</td>
                            <td>{{ productMaterial.description }}</td>
                            <td class="td-actions">
                                <NavLink
                                    :href="
                                        route('product_materials.edit', {
                                            product_material:
                                                productMaterial.id,
                                        })
                                    "
                                ...
                                <section class="delete">
                                    <button
                                        @click="destroy(productMaterial.id)"
        ...
        <div class="pagination">
            <button
                class="btn-class"
                :disabled="!productMaterials.prev_page_url"
                @click.prevent="
                    $inertia.visit(productMaterials.prev_page_url, {
                        preserveState: true,
                        preserveScroll: true,
            ...
            <span
                >Página {{ productMaterials.current_page }} de
                {{ productMaterials.last_page }}</span
            >

            <button
                :disabled="!productMaterials.next_page_url"
                @click.prevent="
                    $inertia.visit(productMaterials.next_page_url, {
                        preserveState: true,
                        preserveScroll: true,
                        data: { perPage: cantidad },
                    })
