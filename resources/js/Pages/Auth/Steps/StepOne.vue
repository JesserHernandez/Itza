...
function updateFieldLocal(field, valueOrEvent) {
    ...
    if (local.value.errors && Object.keys(local.value.errors).length) out.errors = local.value.errors;
...
<template>
    <TitleRegister
        :title="isVendor === true ? '¡Bienvenido a ITZ´AT! Únete a nuestra comunidad de creadores!' : 'Bienvenido a ITZ´AT, la comunidad más grande de artesanos precolombinos.'" />
    <TitleRegister
        :title="isVendor === true ? 'Únete a ITZ´AT como artista. Muestra tu talento al mundo y conéctate con una comunidad vibrante que valora y celebra la riqueza de la artesanía precolombina.' : 'Te damos la bienvenida a ITZ´AT, el lugar donde la tradición y el talento local se encuentran. Apoya a nuestros artesanos nacionales y explora la riqueza de la artesanía precolombina.'" />
    <div class="email-pass">
        <div>
            <InputLabel for="email" value="Correo Electrónico" texAdd=" *" />
            <!-- Soporta ambos: si TextInput expone v-model, usa @update:modelValue; si expone DOM input, @input -->
            <TextInput id="email" :value="local.email" @input="updateFieldLocal('email', $event)"
                @update:modelValue="val => updateFieldLocal('email', val)" type="email" autocomplete="username" />
            <InputError :message="local.errors.email || props.modelValue?.errors?.email" />
        </div>

        <div>
            <InputLabel for="password" value="Password" />
            <TextInput id="password" :value="local.password" @input="updateFieldLocal('password', $event)"
                @update:modelValue="val => updateFieldLocal('password', val)" type="password"
                autocomplete="new-password" />
            <InputError :message="local.errors.password || props.modelValue?.errors?.password" />
        </div>
