...
watch(() => props.modelValue, (nv) => {
    ...
    if (nv.postal_code !== undefined) local.value.postal_code = nv.postal_code;
    if (nv.address !== undefined) local.value.address = nv.address;
}, { deep: true });

// opciones computed
const departmentOptions = computed(() => departments);
const municipalityOptions = computed(() => municipalitiesByDept[local.value.city] || []);
...
<template>
    <TitleRegister title="¡Felicidades! Ahora eres parte de nuestra comunidad" />
    <TitleRegister
    ...
    <div class="step-address">
        ...
        <div class="field">
            ...
            <select id="municipality" :value="local.municipality" @change="e => updateField('municipality', e.target.value)">
        ...
        <div class="field">
            ...
            <TextInput id="postal_code" :value="local.postal_code" @input="e => updateField('postal_code', e)"
                @update:modelValue="val => updateField('postal_code', val)" placeholder="23002" />
        ...
        <div class="field">
            ...
            <TextInput id="address" :value="local.address" @input="e => updateField('address', e)"
                @update:modelValue="val => updateField('address', val)" placeholder="Parque central 2km al norte" />
        ...
        <div class="actions">
            <button type="button" :disabled="!isValid" @click="continueStep">¡Adelante! confirma esta información adicional</button>
        </div>
    </div>
    <div class="img">
